...
BOOL SetPrivilege(void) {
  ...
  if (GetLastError() == ERROR_NOT_ALL_ASSIGNED) {
    printColouredMessage(
        ...
        "you should restart plotter with Administrative rights.",
        RED);
    return FALSE;
  }
  return TRUE;
...
unsigned long long getFreeSpace(const char *path) {
  ...
  GetDiskFreeSpaceExA(path, &lpFreeBytesAvailable, &lpTotalNumberOfBytes,
...
void writer_i(const unsigned long long offset,
              const unsigned long long nonces_to_write,
              const unsigned long long glob_nonces) {
  ...
  if (poc2) {
    ...
    for (size_t scoop = 0; scoop < HASH_CAP / 2; scoop++) {
      for (unsigned long t = 0; t < SCOOP_SIZE * nonces_to_write; t += 64) {
        ...
        memcpy(&cache_write[scoop][t + 32], &cache_write[4095 - scoop][t + 32],
               32);
  ...
  for (size_t scoop = 0; scoop < HASH_CAP; scoop++) {
    ...
    if (!WriteFile(ofile, &cache_write[scoop][0],
                   DWORD(SCOOP_SIZE * nonces_to_write), &dwBytesWritten,
                   nullptr)) {
...
bool write_to_stream(const unsigned long long data) {
  ...
  if (!WriteFile(ofile_stream, &buf, DWORD(sizeof(buf)), &dwBytesWritten,
                 nullptr)) {
...
unsigned long long read_from_stream() {
  ...
  if (!ReadFile(ofile_stream, &buf, DWORD(sizeof(buf)), &dwBytesRead,
                nullptr)) {
...
int drive_info(const std::string &path) {
  ...
  DWORD MCLength;
  DWORD FileSF;
  if (GetVolumeInformationA(drive.c_str(), NameBuffer, sizeof(NameBuffer),
...
/*
unsigned long long GCD(unsigned long long a, unsigned long long b)  //наибольший
общий делитель
{
        return b ? GCD(b, a%b) : a;
}

unsigned long long LCM(unsigned long long a, unsigned long long b) //наименьшее
общее кратное
{
        return a / GCD(a, b) * b;
}
...
int main(int argc, char *argv[]) {
  ...
  std::string out_path = "";
  std::string version = "v1.31_SSE";
  std::string supportInstruction;
  ...
  if (cpuInfo.hasSSE41()) {
    supportInstruction += "SSE4.1 ";
  }
  if (cpuInfo.hasSSE42()) {
    supportInstruction += "SSE4.2 ";
  }
  if (cpuInfo.hasAVX()) {
    supportInstruction += "AVX ";
  }
  if (cpuInfo.hasAVX2()) {
    supportInstruction += "AVX2 ";
  }
  ...
  printf("\nXPlotter for BURST\n");
  printf("\nRun on %s\n", cpuInfo.model().c_str());
  printf("\nSupport instructions %s\n", supportInstruction.c_str());
  ...
  if (argc < 2) {
    ...
    printf(
        "                         If this is your first HDD then set it to 0, "
        "other wise set it to your last hdd's <start_nonce> + <nonces>\n");
    printf(
        "         <nonces> = how many nonces you want to plot - 200gb is about "
        "800000 nonces.\n");
    printf(
        "                         If not specified the number of nonces, or it "
        "is 0, the file is created on all the free disk space.\n");
    ...
    printf(
        "\nExample:\n %s -id 17930413153828766298 -sn 600000000 -n 1000000 -t "
        "6 -path D:\\plots\n",
        argv[0]);
    exit(-1);
  }
  ...
  for (size_t i = 1; i < args.size(); i++) {
    ...
    if (args[i] == "-low") {
      if (!SetPriorityClass(GetCurrentProcess(), BELOW_NORMAL_PRIORITY_CLASS)) {
        ...
        printf("Set priority 'below normal' - Ok\n");
    }
  }
  ...
  if (!GetDiskFreeSpaceA(out_path.c_str(), &sectorsPerCluster, &bytesPerSector,
  ...
  // ajusting nonces
  nonces =
      (nonces / (bytesPerSector / SCOOP_SIZE)) * (bytesPerSector / SCOOP_SIZE);

  std::string filename = "";
  if (poc2) {
    filename = std::to_string(addr) + "_" + std::to_string(startnonce) + "_" +
               std::to_string(nonces);
  } else {
    filename = std::to_string(addr) + "_" + std::to_string(startnonce) + "_" +
               std::to_string(nonces) + "_" + std::to_string(nonces);
  }
  supports_streams = drive_info(out_path);
  BOOL granted = SetPrivilege();

  if (supports_streams == 1) {
    ofile_stream = CreateFileA(
        (out_path + filename + ":stream").c_str(), GENERIC_WRITE | GENERIC_READ,
  ...
  // check free RAM
  if (freeRAM < nonces_per_thread * threads * PLOT_SIZE * 2)
    nonces_per_thread = freeRAM / threads / PLOT_SIZE / 2;

  // ajusting
  nonces_per_thread = (nonces_per_thread / (bytesPerSector / SCOOP_SIZE)) *
                      (bytesPerSector / SCOOP_SIZE);
  ...
  for (size_t i = 0; i < HASH_CAP; i++) {
    cache[i] =
        (char *)VirtualAlloc(nullptr, threads * nonces_per_thread * SCOOP_SIZE,
  ...
  printf("Using %llu Mb of %llu Mb free RAM\n",
         nonces_per_thread * threads * 2 * PLOT_SIZE / 1024 / 1024,
         freeRAM / 1024 / 1024);
  if (nonces_done > 0) {
    SetConsoleTextAttribute(hConsole, colour::GREEN);
  ...
  while (nonces_done < nonces) {
    ...
    if (leftover / (nonces_per_thread * threads) == 0) {
      if (leftover >= threads * (bytesPerSector / SCOOP_SIZE)) {
        ...
        // ajusting
        nonces_per_thread =
            (nonces_per_thread / (bytesPerSector / SCOOP_SIZE)) *
            (bytesPerSector / SCOOP_SIZE);
      } else {
        threads = 1;
    ...
    for (size_t i = 0; i < threads; i++) {
      ...
      if (cpuInfo.hasAVX2()) {
        th = std::thread(AVX2::work_i, i, addr,
                         startnonce + nonces_done + i * nonces_per_thread,
                         nonces_per_thread);
      } else if (cpuInfo.hasAVX()) {
        th = std::thread(AVX1::work_i, i, addr,
                         startnonce + nonces_done + i * nonces_per_thread,
                         nonces_per_thread);
      } else {
        th = std::thread(SSE4::work_i, i, addr,
                         startnonce + nonces_done + i * nonces_per_thread,
                         nonces_per_thread);
      }
    ...
    printf("\r[%llu%%] Generating nonces from %llu to %llu\t\t\t\t\t\t\n",
           (nonces_done * 100) / nonces, startnonce + nonces_done,
           startnonce + nonces_done + nonces_in_work);
    ...
    do {
      ...
      printf("\r[%llu%%] CPU: %llu%% done, (%llu nonces/min)",
             (nonces_done * 100) / nonces, x * 100 / nonces_in_work,
             x * 60000 / (GetTickCount64() - t_timer));
      printf("\t\tHDD: Writing scoops: %.2f%% %.2f MB/s",
             (double)(written_scoops * 100) / (double)HASH_CAP,
  ...
  while ((written_scoops != 0) && (written_scoops < HASH_CAP)) {
    Sleep(100);
    printf("\r[%llu%%] CPU: %llu nonces done                   ",
           (nonces_done * 100) / nonces, nonces_done + x);
    printf("\t\tHDD: Writing scoops: %.2f%%",
           (double)(written_scoops * 100) / (double)HASH_CAP);
  }
  printf(
      "\rWait, closing file...                                                 "
