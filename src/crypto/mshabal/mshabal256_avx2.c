...
static void
mshabal256_compress(mshabal256_context *sc, const unsigned char *buf0,
...
#define PP256(xa0, xa1, xb0, xb1, xb2, xb3, xc, xm)                            \
  do {                                                                         \
    ...
    tt =                                                                       \
        _mm256_xor_si256(_mm256_xor_si256(tt, xb1),                            \
...
void mshabal256_init(mshabal256_context *sc, unsigned out_size) {
  ...
  mshabal256_compress(sc, sc->buf0, sc->buf1, sc->buf2, sc->buf3, sc->buf4,
                      sc->buf5, sc->buf6, sc->buf7, 1);
  for (u = 0; u < 16; u++) {
...
/* see shabal_small.h */
void mshabal256_close(mshabal256_context *sc, unsigned ub0, unsigned ub1,
                      unsigned ub2, unsigned ub3, unsigned ub4, unsigned ub5,
                      unsigned ub6, unsigned ub7, unsigned n, void *dst0,
