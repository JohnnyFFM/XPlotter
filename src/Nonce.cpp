#include "Nonce.h"
#include "crypto/mshabal/mshabal.h"
#include "crypto/mshabal/mshabal256.h"
#include "crypto/shabal/shabal.h"
#include <intrin.h>

namespace AVX1 {
void work_i(const size_t local_num, unsigned long long loc_addr,
            unsigned long long local_startnonce,
  ...
  int8_t *xv = reinterpret_cast<int8_t *>(
      &loc_addr); // optimization: reduced 8*4*nonces assignments
  ...
  for (unsigned long long n = 0; n < local_nonces;) {
    if (n + 4 <= local_nonces) {
      ...
      for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE) {
        memcpy(mx, init_mx,
               sizeof(*init_mx)); // optimization: avx1_mshabal_init(mx, 256);
        if (i < PLOT_SIZE + 16 - HASH_CAP)
          len = HASH_CAP; // optimization: reduced 8064 assignments
        else
          len = PLOT_SIZE + 16 - i;
        avx1_mshabal(mx, &gendata1[i], &gendata2[i], &gendata3[i], &gendata4[i],
                     len);
        avx1_mshabal_close(mx, 0, 0, 0, 0, 0, &gendata1[i - HASH_SIZE],
      ...
      for (size_t i = 0; i < HASH_CAP; i++) {
        memmove(&cache[i][(n + 0 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata1[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 1 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata2[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 2 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata3[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 3 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata4[i * SCOOP_SIZE], SCOOP_SIZE);
      }
      n += 4;
    } else {
      _mm256_zeroupper();
      ...
      for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE) {
        memcpy(x, init_x,
               sizeof(*init_x)); // optimization: shabal_init(x, 256);
        if (i < PLOT_SIZE + 16 - HASH_CAP)
          len = HASH_CAP; // optimization: reduced 8064 assignments
        else
          len = PLOT_SIZE + 16 - i;
      ...
      // Sort them:
      for (size_t i = 0; i < HASH_CAP; i++)
        memmove(&cache[i][(n + local_num * local_nonces) * SCOOP_SIZE],
                &gendata[i * SCOOP_SIZE], SCOOP_SIZE);
      n++;
    }
...
namespace AVX2 {
void work_i(const size_t local_num, unsigned long long loc_addr,
            const unsigned long long local_startnonce,
            const unsigned long long local_nonces) {
  unsigned long long nonce;
  unsigned long long nonce1;
  unsigned long long nonce2;
  ...
  mshabal256_context *init_mx =
      new mshabal256_context[sizeof(mshabal256_context)];
  ...
  for (unsigned long long n = 0; n < local_nonces;) {
    if (n + 8 <= local_nonces) {
      ...
      for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE) {
        memcpy(mx, init_mx,
               sizeof(*init_mx)); // optimization: mshabal256_init(mx, 256);
        if (i < PLOT_SIZE + 16 - HASH_CAP)
          len = HASH_CAP; // optimization: reduced 8064 assignments
        else
          len = PLOT_SIZE + 16 - i;
        mshabal256(mx, &gendata1[i], &gendata2[i], &gendata3[i], &gendata4[i],
                   &gendata5[i], &gendata6[i], &gendata7[i], &gendata8[i], len);
        mshabal256_close(mx, 0, 0, 0, 0, 0, 0, 0, 0, 0,
                         &gendata1[i - HASH_SIZE], &gendata2[i - HASH_SIZE],
                         &gendata3[i - HASH_SIZE], &gendata4[i - HASH_SIZE],
                         &gendata5[i - HASH_SIZE], &gendata6[i - HASH_SIZE],
                         &gendata7[i - HASH_SIZE], &gendata8[i - HASH_SIZE]);
      }

      memcpy(mx, init_mx,
             sizeof(*init_mx)); // optimization: mshabal256_init(mx, 256);
      mshabal256(mx, gendata1, gendata2, gendata3, gendata4, gendata5, gendata6,
                 gendata7, gendata8, 16 + PLOT_SIZE);
      mshabal256_close(mx, 0, 0, 0, 0, 0, 0, 0, 0, 0, final1, final2, final3,
                       final4, final5, final6, final7, final8);
      ...
      for (size_t i = 0; i < HASH_CAP; i++) {
        ...
        memmove(&cache[i][(n + 3 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata4[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 4 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata5[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 5 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata6[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 6 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata7[i * SCOOP_SIZE], SCOOP_SIZE);
        memmove(&cache[i][(n + 7 + local_num * local_nonces) * SCOOP_SIZE],
                &gendata8[i * SCOOP_SIZE], SCOOP_SIZE);
      }
      n += 8;
    } else {
      _mm256_zeroupper();
      ...
      for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE) {
        memcpy(x, init_x,
               sizeof(*init_x)); // optimization: shabal_init(mx, 256);
        if (i < PLOT_SIZE + 16 - HASH_CAP)
          len = HASH_CAP; // optimization: reduced 8064 assignments
        else
          len = PLOT_SIZE + 16 - i;
...
void work_i(const size_t local_num, unsigned long long loc_addr,
  ...
  for (unsigned long long n = 0; n < local_nonces;) {
    if (n + 4 <= local_nonces) {
      ...
      for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE) {
        memcpy(mx, init_mx,
               sizeof(*init_mx)); // optimization: sse4_mshabal_init(mx, 256);
        if (i < PLOT_SIZE + 16 - HASH_CAP)
          len = HASH_CAP; // optimization: reduced 8064 assignments
        else
          len = PLOT_SIZE + 16 - i;
        sse4_mshabal(mx, &gendata1[i], &gendata2[i], &gendata3[i], &gendata4[i],
                     len);
        sse4_mshabal_close(mx, 0, 0, 0, 0, 0, &gendata1[i - HASH_SIZE],
                           &gendata2[i - HASH_SIZE], &gendata3[i - HASH_SIZE],
                           &gendata4[i - HASH_SIZE]);
      }
